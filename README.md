...
## 🚀 Features
- **Queue-based processing** of hardware items (Drills, Ladders, Hammers, Paint Brushes)  
- **Real-time console updates** showing items added to the queue and their processing status  
- Batch processing of items (processes automatically when a batch size is reached)  
- Displays item details: `Id`, `Name`, `Type`, `Quantity`, and `UnitValue`  
...
## 📖 How It Works
1. Hardware items (`Drill`, `Hammer`, `Ladder`, `PaintBrush`) are added to a `CustomQueue`.  
...
4. Item details (ID, name, type, quantity, value) are printed neatly in a tabular format in the console.  
